        ...
        >
          ...
          <template v-if="isActive(v.path)">
            ...
            <i
              class="
                el-icon-close
                layout-navbars-tagsview-ul-li-icon layout-icon-active
              "
              v-if="!v.meta.isAffix"
          ...
          <i
            class="
              el-icon-close
              layout-navbars-tagsview-ul-li-icon layout-icon-three
            "
            v-if="!v.meta.isAffix"
            @click.stop="closeCurrentTagsView(v.path)"
          ></i>
...
export default {
  ...
  setup() {
    ...
    const getTagsViewRoutes = () => {
      ...
      state.tagsViewList = [];
      if (!store.state.themeConfig.themeConfig.isCacheTagsView)
        Session.remove("tagsViewList");
      state.tagsViewRoutesList = store.state.permissionRoute.topbarRouters;
      initTagsView();
    };
    // vuex 中获取路由信息：如果是设置了固定的（isAffix），进行初始化显示
    const initTagsView = () => {
      if (
        Session.get("tagsViewList") &&
        store.state.themeConfig.themeConfig.isCacheTagsView
      ) {
    ...
    watch(store.state, (val) => {
      // console.log("监听路由的变化1",val.permissionRoute.topbarRouters);
      // console.log("监听路由的变化2",state.tagsViewRoutesList);
      if (
        val.permissionRoute.topbarRouters.length ===
