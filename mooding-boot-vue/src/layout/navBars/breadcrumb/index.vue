	...
	setup() {
		...
		const setFilterRoutes = () => {
			...
			if (layout === 'classic' && isClassicSplitMenu) {
				state.menuList = delClassicChildren(filterRoutesFun(store.state.permissionRoute.sidebarRouters));
				const resData = setSendClassicChildren(route.path);
				proxy.mittBus.emit('setSendClassicChildren', resData);
			} else {
				state.menuList = filterRoutesFun(store.state.permissionRoute.sidebarRouters);
			}
		};
		...
		const setSendClassicChildren = (path: string) => {
			...
			let currentData: any = {};
			filterRoutesFun(store.state.permissionRoute.sidebarRouters).map((v, k) => {
